...
const storage: StorageEngine = multer.diskStorage({
  destination: (request: Request, file: Express.Multer.File, cb) => {
    cb(null, path.join(__basedir, "/assets/images/"));
  },
  filename: (request: Request, file: Express.Multer.File, cb) => {
    const uploadPath = path.join(__basedir, "/assets/images/");
    ...
    if (previousFileName && previousFileName != "") {
      ...
      fs.unlink(previousFilePath, (err) => {
        if (err && err.code !== "ENOENT") {
          // 'ENOENT' significa "archivo no encontrado"
          console.error("Error al intentar borrar el archivo anterior:", err);
        } else if (!err) {
          console.log(
            `Archivo anterior ${previousFileName} borrado exitosamente.`
          );
        }
      });
    }
    ...
    const finalName = `${nombreBase}${ext}`;
    cb(null, finalName);
  },
});
